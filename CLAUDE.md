...
テスト・境界検査は **Docker（Docker Compose）で実行する**。ローカルに Python / uv を導入する必要はない（uv は Docker イメージ内部でのみ使う。詳細は [README の開発セットアップ](./README.md#開発セットアップ)、構成根拠は [ADR-0005](./docs/adr/0005-docker-test-environment.md)）。
...
- [.claude/skills/create-issue/SKILL.md](./.claude/skills/create-issue/SKILL.md) — Issue を対話形式で起票するスキル
