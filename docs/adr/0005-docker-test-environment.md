# ADR-0005: テスト実行環境を Docker（Compose）で提供する
...
- **実行**: `docker compose run --rm test`（pytest）/ `docker compose run --rm lint`（lint-imports）。実行時は `uv run --no-sync` でネットワーク不要・即時。テスト実行は Docker に一本化し、ローカルに uv を入れての実行は前提としない。
