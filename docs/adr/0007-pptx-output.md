...
- **ステータス**: `置き換え済み`（[ADR-0013](./0013-deprecate-presentation-adopt-claude-design.md) — presentation 領域を廃止し Claude Design へ移行）
- **決定日**: 2026-06-18
- **決定者**: masaya_ueki
...
- **単一の真実**: `outline.yml`（`domains/presentation/README.md`）は不変だった。HTML と pptx は同じ契約を読む別レンダラだった。
- **マッピング仕様**: expression→ネイティブ pptx の対応は `scripts/deckgen/README.md` に定義していた。索引スキル `.claude/skills/slide-pptx/` からも参照していた。
- **生成物**: 出力 `.pptx` はビルド成果物として `.gitignore` で除外していた（[R-STRUCT-4](../../rule/directory-structure.md)）。deck はコマンドで都度再生成していた。
- **割り切り**: アニメ・スピーカーノート・高度チャート非対応。完全ブランド再現は `--template` 運用。図解（matrix-2x2 / tree / pyramid / venn）はネイティブ図形で描き（#32 で tree のコネクタ線・venn の重なり円を作り込み）、表現の限界を超えるものは箇条書きにフォールバックしていた。
- **ADR-0003 との関係**: ADR-0003 の HTML パイプラインは無改変で併存していた。本 ADR は出力ターゲットを追加するのみだった。`presentation/` の「コード無し content 領域」という位置づけも維持していた（コードは `scripts/` 側に置いていたため）。
- **廃止**: [ADR-0013](./0013-deprecate-presentation-adopt-claude-design.md) により `domains/presentation/` と `scripts/deckgen/` は削除済み。

## 関連ドキュメント・リンク

- [ADR-0013](./0013-deprecate-presentation-adopt-claude-design.md) — 本 ADR を置き換え、presentation 領域を廃止し Claude Design へ移行する決定
