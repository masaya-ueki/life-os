...
> **採用理由**: [ADR-0016](../adr/0016-claude-design-design-system-file.md)（前提となる移行判断は [ADR-0013](../adr/0013-deprecate-presentation-adopt-claude-design.md)。指示書の形式は [ADR-0017](../adr/0017-slide-spec-claim-materials-deck-types.md)）
...
| `decks/<案件名>/` | **指示書と素材**。案件ごとの構成・ページ上限・ページごとの主張と材料と、画像 / xlsx などの元資料 | 案件ごとに増える |
...
| **指示書** | 何を何ページで話すか（構成・ページ上限・主張と材料） | `decks/<案件名>/spec.yml`（`slide-spec-writer` スキルが生成） |
...
3. Claude Design に **デザインシステム ＋ spec.yml ＋ `deck_type` に対応する型ファイル（`.claude/skills/slide-spec-writer/references/deck-types/<種類>.md`）＋ `materials/` の素材** を渡す（型ファイルは該当する 1 つだけ。`deck_type` が空なら渡さない）
...
- 資料タイプの型（ハンズオンなど）を増やすときは、机上で作らず Claude Design の良い出力から逆算して [`slide-spec-writer`](../../.claude/skills/slide-spec-writer/SKILL.md) の `references/deck-types/` に追加する（手順はスキルの「新しい種類の資料の運用」、根拠は [ADR-0017](../adr/0017-slide-spec-claim-materials-deck-types.md)）。案件ごとの `spec.yml` に独自の型を書かない
- 指示書のキー（形式）を変えるときは `assets/slide_spec_template.yml` を直す
