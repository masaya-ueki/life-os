...
├── domains/                  # 領域コンテナ: Bounded Context をまとめる親（ADR-0008）
...
│   └── skills/               # Claude Code スキル（create-issue / slide-* / code-review-* / directory-keeper）
├── .github/
...
各領域はスケルトン段階で、要件が固まり次第 `public.py` と内部を肉付けしていく。新しい領域を追加する際は、`domains/` 配下のディレクトリ・uv workspace の `members`・`.importlinter` のコントラクト・対応する `system: *` ラベルを併せて整備する（手順は ADR-0002、配置先は [ADR-0008](./docs/adr/0008-group-domains-under-domains-dir.md) 参照）。
