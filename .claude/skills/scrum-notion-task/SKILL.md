...
```
{プロジェクト名}{半角空白}{PBIを簡潔に表したラベル}|{タスク名}
```

例: `JMAS AI WBS作成|リリース`（親 PBI が `20260803 JMAS AI WBS作成` の場合）

- 「プロジェクト名」は親 PBI の Project ページの `Project Name` をそのまま使う（PBI の Title に含まれるプロジェクト名と同じ）。
...
- 同一 PBI 配下の複数 Task では、「プロジェクト名 + ラベル」部分をすべて揃える。
- プロジェクト名とラベルの間は半角空白、ラベルとタスク名の区切り文字は半角 `|`。
