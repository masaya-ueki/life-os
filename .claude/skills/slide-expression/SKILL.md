...
renderer（`slide-html-renderer`）はこの規約に従って自己完結 HTML を生成する。配色トークンの単一ソースは [`domains/presentation/templates/theme-tokens.yml`](../../../domains/presentation/templates/theme-tokens.yml)（HTML/pptx 共有）、レイアウト・印刷の CSS 方針は [`domains/presentation/templates/base.css.md`](../../../domains/presentation/templates/base.css.md) を参照。
...
4. **配色トークン**: CSS 変数（`--bg`, `--fg`, `--accent`, `--muted` 等）で `deck.theme` を切替可能にする。色値の単一ソースは `domains/presentation/templates/theme-tokens.yml`。
