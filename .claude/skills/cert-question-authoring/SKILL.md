...
全ジャンルを回すときは、各ジャンルを genre-doc-map の被覆表に対して「1 トピック = 最低 1 問」で埋め、最後に**被覆クロスチェック**を実行する。**完了条件は件数の下限ではなく、この被覆表（トップレベルツリー節の 5 URL を除く各 `source_url`）が問題で使われていること**。

```bash
# 被覆チェック: genre-doc-map の各トピックURLが問題の source_url に現れるか
python3 - <<'PY'
import json, re
d=json.load(open("domains/certification/data/snowpro_core.json"))
used={q["source_url"] for q in d["questions"]}
tree={"https://docs.snowflake.com/en/guides-overview","https://docs.snowflake.com/en/sql-reference-commands",
      "https://docs.snowflake.com/en/developer","https://docs.snowflake.com/en/reference",
      "https://docs.snowflake.com/en/release-notes/overview"}
m=open(".claude/skills/cert-question-authoring/references/genre-doc-map.md").read()
topic=[u for u in dict.fromkeys(re.findall(r"https://docs\.snowflake\.com/[^\s|)]+", m)) if u not in tree]
print("uncovered:", [u for u in topic if u not in used] or "NONE")
PY
```

> 複数ジャンルを並行で作る場合、サブエージェントには**各ジャンルの問題を一時ファイルへ返させ**、呼び出し側が採番・検証・整合性ゲートを中央で回すと、同一 JSON への同時書き込み競合を避けられる（本基盤の #98 ループで用いた運用）。
