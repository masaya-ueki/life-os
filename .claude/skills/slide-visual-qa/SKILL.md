...
品質検査は **決定的 QA（`deckgen.qa`）→ PNG 目視** の二段で行う。

| 段 | 担当 | 検出するもの |
|----|------|-------------|
| 1. 決定的 QA（Step 0） | `scripts/deckgen/src/deckgen/qa.py`（機械） | 最小フォント割れ・型スケール逸脱・色トークン外の文字色・境界超過・塗り形状の過剰重なり・フォントサイズ種過多・テキスト量超過・WCAG コントラスト・プレースホルダ・content 過多 |
| 2. PNG 目視（Step 1〜） | モデル（判断） | 表現の適切さ・整列・余白バランス・図解の意図どおりの見え方など、機械判定できない審美・意味の問題 |

機械で検出できる崩れを目視に持ち込まない（トークンと見逃しの両方を減らす）。
**Step 0 の error が残っている間は PNG 変換に進まない。**

---

## Step 0: 決定的 QA（deckgen.qa）を先に実行

```bash
# Docker（推奨・環境差異なし）
docker compose run --rm test uv run --directory scripts/deckgen -m deckgen.qa <slug>
# uv がホストにある場合
uv run --project scripts/deckgen -m deckgen.qa <slug>
# warning もゲートにする場合は --strict、機械処理には --json
```

- **error**: outline.yml / 生成物の修正が必要。修正 → 再生成 → 再実行で 0 にしてから先へ進む
- **warning**: 内容を確認し、A 種（outline で直せる）はこの場で修正、B 種（deckgen コード起因）は
  Step 4 の B 種リストに記録して先へ進んでよい
- 既知の許容 warning（例: チャートのみのスライドに出る `QA-PPTX-EMPTY`）は理由を添えてスキップ可
...
| 整列（グリッド整合） | スライド内の要素の左端・上端が揃っている（カード列・箇条書き・見出しの縦ラインが一直線） |
...
| スライド間の整列一貫性 | タイトル・下線・リード・本文開始位置がスライドをまたいで同じ位置にある（表紙の左端もコンテンツと一直線） |
| カードスタイル統一 | カード風の面（比較カラム / KPI / フロー / ツリー / マトリクス）の角丸・枠線・塗りが揃っている |
