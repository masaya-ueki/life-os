...
新規 instinct を作成するときの初期値は source で決まる：

| source | 初期 confidence | 理由 |
|--------|----------------|------|
| `session-observation` | **0.3** | 主観的な気づき。再観測で検証する |
| `pr-review` | **0.5** | PR プロセスで外部検証済みのため中程度からスタート |
| `user-correction` | **0.3** | 訂正は降格イベント。新規登録でも暫定スタート |

### 昇格条件（明確な基準）

各段階への昇格には、以下の条件を**すべて**満たす必要がある：

| 現在 | 次段階 | 必要条件 |
|------|--------|---------|
| 0.3 | 0.5 | ① `observation_count ≥ 2`（同一 ID で再観測） |
| 0.5 | 0.7 | ① `observation_count ≥ 3` かつ ② evidence に `source: pr-review` または `source: user-correction` が **1 件以上** |
| 0.7 | 0.9 | ① `observation_count ≥ 4` かつ ② evidence に `source: user-correction` が **0 件**（一度も訂正されていない） |

**意図**:
- `0.3 → 0.5`: 「1 度見た」から「繰り返し出てきた」へ。再観測のみで十分。
- `0.5 → 0.7`: 「主観的な気づき」から「外部で確認されたパターン」へ。PR レビュー等の外部根拠が必要。
- `0.7 → 0.9`: 「強い」から「ほぼ確定」へ。訂正歴がないことで信頼性を担保。

### 同一 ID 判定ルール

- `id:` フィールドの**完全一致のみ**を「同一 ID」と判定する
- セマンティック類似による自動マッチングは行わない
- `/learn` 実行時、Claude は既存 instinct をスキャンして ID が近いものを候補として提示する
- **ユーザーが既存 ID の再利用を選んだ場合のみ** `observation_count` をインクリメントする
- ユーザーが「新規」を選んだ場合は confidence 0.3（または source に応じた初期値）で新規作成

### 降格・停止条件

| イベント | 変動 |
|---------|------|
| ユーザーが訂正（`source: user-correction` の evidence 追加） | **1 段階降格**（0.9→0.7→0.5→0.3。0.3 が下限） |
| 30 日間更新なし | **MVP では未実装**（`/instinct-status --orphaned` で表示のみ。v2 で実装） |
