...
## 入力
- `domains/presentation/decks/{slug}/outline.yml` のパス（または slug）
...
3. 配色は **`domains/presentation/templates/theme-tokens.yml`**（HTML/pptx 共有の単一ソース）を読む。レイアウト・印刷の CSS 方針は **`domains/presentation/templates/base.css.md`** を読んで踏襲する。
...
5. `domains/presentation/decks/{slug}/index.html` に **UTF-8・自己完結 HTML** で書き出す。
